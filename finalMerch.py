...
app = Flask(__name__)
CORS(app, resources={r"/upload": {"origins": ["http://10.22.130.241:8081", "http://10.22.130.241:5005"]}})
...
def process_image(filepath):
    ...
    det_model = YOLO("models/best (1).pt")
    label_map = det_model.model.names

    # Initialize OpenVINO Core
    core = ov.Core()
    det_ov_model = core.read_model("models/best (1).xml")
    ...
    output_path = Path("json/detectionsMerch.json")
    print(f"Saving detections to: {output_path}")  # Debug info
    with open(output_path, "w") as f:
        json.dump(detections, f, indent=4)

    # Save the image with detections
    output_image_path = Path("images/detectionsMerch.jpg")
...
def upload_file():
    ...
    try:
        ...
        detections_path = "json/detectionsMerch.json"
        image_path = "images/detectionsMerch.jpg"
